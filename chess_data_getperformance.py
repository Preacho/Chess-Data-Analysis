...
def calculate_eval(current_eval, previous_eval, turn):
    if('#' in str(current_eval) or '#' in str(previous_eval)):
        ...
        if(float(current_eval) > 0.0):
            return 1, turn%2
        return 0, turn%2

    if(float(current_eval)  - float(previous_eval) > 0.0):
        return 1, turn%2
    return 0, turn%2
...
def get_game_performance(game):
    ...
    dataframe['Improvement'] = dataframe.apply(lambda x: calculate_eval(x['Eval'],x['Previous_Eval'], x['Turn'])[0], axis = 1)
...
def main():
    ...
    plt.grid(True)
    plt.plot(game_performance_white['calc_time'], game_performance_white['Improvement'])
